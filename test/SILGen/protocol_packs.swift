// RUN: %target-swift-emit-silgen-ossa -o /dev/null -enable-sil-opaque-values -target %target-swift-5.9-abi-triple -module-name test %s
// RUN: %target-swift-emit-sil -sil-verify-all -o /dev/null -enable-sil-opaque-values -target %target-swift-5.9-abi-triple -module-name test %s
